  ...
  // printf("curr: %d\n",curr->size_status);
   ...
   // best=1;
    ...
    if (b->size_status-size>=12){
      ...
      b_new->size_status=(b->size_status)-size-sizeof(block_header);
      b_new->next=b->next;
      b->next=b_new;
    }
...
{
  /* Your code should go in here */
  block_header* thisblock = list_head;
  thisblock->next = list_head->next;
  block_header* prevblock = list_head;
  prevblock->next = list_head->next;
  ...
  if((thisblock->size_status)% 4  == 1) {
  thisblock->size_status = thisblock->size_status - 1;
  printf("block of size %d freed.\n", thisblock->size_status);
    if(prevblock->size_status %  4 == 0){
      printf("previous block empty. coalescing...\n");
      printf("previous block size: %d \n", prevblock->size_status);
      prevblock->size_status = prevblock->size_status + thisblock->size_status + sizeof(block_header);
      printf("new block size: %d \n", prevblock->size_status);
      prevblock->next = thisblock->next;
      thisblock->size_status = NULL;
    }
    else if(prevblock->size_status % 4  == 1){
      printf("previous block not empty. \n");
    }

    if((thisblock->next)->size_status % 4 == 0){
      printf("next block empty. coalescing...\n");
      printf("next block size: %d \n", (thisblock->next)->size_status);
      if(thisblock->size_status == NULL){
        printf("this block coalesced with previous block. using coalesced block information.\n");
        prevblock->size_status = prevblock->size_status + (thisblock->next)->size_status+sizeof(block_header);
      	prevblock->next=(thisblock->next)->next;
	}
      else{
        thisblock->size_status = sizeof(block_header)+ thisblock->size_status + (thisblock->next)->size_status;
      	thisblock->next=(thisblock->next)->next;
	}
    }
    else if((thisblock->next)->size_status % 4  == 1){
      printf("next block not empty.\n");
    }
  }else printf("this block is already free.\n");
}
